...
import { Check, Plus, PlusCircleIcon, XIcon } from 'lucide-react';
import MooIcon from './MooIcon';
import { useSession } from 'next-auth/react';
import CreateAIModal from './CreateAIModal';
import { CustomAISkeleton } from './CustomAISkeleton';
...
export default function PersonalitySelector({
  ...
  const handleDeleteSingleAI = async (aiId: string) => {
    if (!session?.user?.id) return;

    // 확인 다이얼로그
    const confirmed = confirm('해당 moo를 삭제하시겠습니까?');
    if (!confirmed) return;

    setDeleting(aiId); // 삭제 중 상태 설정

    try {
      const response = await fetch(`/api/custom-ai/${aiId}`, {
        method: 'DELETE',
        headers: {
      ...
      if (response.ok) {
        ...
        // 삭제된 AI가 현재 선택된 AI라면 기본 AI로 변경
        if (selectedId === aiId) {
          onPersonalityChange(AI_PERSONALITIES[0]); // 첫 번째 기본 AI로 변경
        }

        fetchCustomAIs(); // 목록 새로고침
      } else {
      ...
      <div>
        <div className="flex flex-row items-center justify-between">
          ...
          <PlusCircleIcon
            ...
            className="w-6 h-6 font-bold text-green-500 mb-3 cursor-pointer"
          />
          ...
          <div className="space-y-3">
            {customAIs.map((customAI) => {
              ...
              try {
                ...
                throw new Error('MBTI 파싱 오류: ' + error);
              }
              const texts = generateMbtiTexts(mbtiTypes);
              ...
              return (
                <div
                  key={customAI.id}
                  className={`
                    relative bg-white/60 rounded-xl p-5 transition-all duration-200
                    ${
                    ...
                    ${
                      deleting === customAI.id ? 'opacity-50' : 'cursor-pointer'
                    }
                  `}
                  onClick={(e) => {
                    // 삭제 버튼이나 삭제 버튼 내부 요소 클릭 시 선택 방지
                    ...
                    // 삭제 중일 때도 선택 방지
                    if (deleting === customAI.id) {
                      e.preventDefault();
                      e.stopPropagation();
                      return;
                    }
                    handleCustomAISelect(customAI);
                  }}
                  onMouseDown={(e) => {
                    // 삭제 버튼 클릭 시 mousedown도 차단하여 active:scale-95 방지
                    const target = e.target as HTMLElement;
                        ...
                        >
                          <button
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              handleDeleteSingleAI(customAI.id);
                            }}
                            onMouseDown={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                            ...
                            disabled={deleting === customAI.id}
                            className={`p-2 rounded-full transition-colors mb-3 ${
                              deleting === customAI.id
